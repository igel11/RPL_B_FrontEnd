...
const Laporan = () => {
  ...
  // Ambil data pengguna setelah login (mendapatkan dari localStorage)
  useEffect(() => {
    const storedUserName = localStorage.getItem("userName");
    const storedSubRole = localStorage.getItem("subRole");

    if (storedUserName && storedSubRole) {
      setUserName(storedUserName);
      setSubRole(storedSubRole);
    } else {
      // Jika tidak ada data pengguna di localStorage, coba login
      const getUserData = async () => {
        const email = "ade@example.com"; // Ganti dengan email yang digunakan untuk login
        const password = "ade_tes"; // Ganti dengan password yang digunakan
        ...
        try {
          ...
          // Menyimpan userName dan subRole di localStorage
