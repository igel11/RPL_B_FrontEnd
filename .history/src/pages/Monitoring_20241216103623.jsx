...
const Monitoring = () => {
  ...
  useEffect(() => {
    ...
    const fetchLabItems = async () => {
      try {
        const response = await axios.get(`http://localhost:3500/api/monitoring?lab_name=${selectedLab}`);
            ...
            {labs.map((lab) => (
              <button
                ...
                className={`
                  ...
                  ${selectedLab === lab ? "bg-black text-white" : "bg-gray-100 hover:bg-gray-200 text-gray-700"}
                `}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {labItems.length === 0 ? (
              <p className="text-center text-gray-500">
                Tidak ada alat di laboratorium ini
              </p>
            ) : (
              labItems.map((item, index) => (
                <div key={index} className="flex items-center bg-gray-50 p-4 rounded-lg">
                  <div className={`mr-4 ${item.iconBg || "bg-gray-300"} p-3 rounded-full`}>
                    <span className={`text-2xl ${item.iconColor || "text-black"}`}>
                      {item.icon || "⚙️"}
                    </span>
                  </div>
                  <div className="flex-grow">
                    ...
                    <div className="flex space-x-2 mt-1">
                      <span className="text-green-600 text-sm">Tersedia: {item.available}</span>
                      <span className="text-red-600 text-sm">Rusak: {item.broken}</span>
                      <span className="text-yellow-600 text-sm">Perbaikan: {item.under_repair}</span>
                      <span className="text-black text-sm">Total: {item.total}</span>
                    </div>
                  </div>
                </div>
              ))
