...
const SignIn = () => {
  ...
  const handleLogin = async (e) => {
    ...
    const email = e.target.email.value; // Mengambil email dari input form
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error('Backend Error:', data.message); // Tambahkan log error
        alert('Login failed: ' + data.message);
      }
    } catch (error) {
      console.error('Network Error:', error); // Tambahkan log error
      ...
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-sm">
        ...
        <form onSubmit={handleLogin}>
          <div className="flex items-center border border-gray-300 rounded-md mb-6">
            <input
              type="email"
              name="email" // Menggunakan input type email
              placeholder="Email"
